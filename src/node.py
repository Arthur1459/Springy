...
class Node:
    def __init__(self, coordi, speedi=(0, 0, 0), acceli=(0, 0, 0)):
        self.id = u.getNewId()
        self.tags = {'node'}

        self.coord = list(coordi)
        self.coord_at_last_update = coordi
        self.speed_pushed = list(speedi)
        self.accel = list(acceli)
...
class SolidNode(Node):
    def __init__(self, coordi, sizei, speedi=(0, 0, 0), acceli=(0, 0, 0)):
        super().__init__(coordi, speedi, acceli)
    ...
    def update(self):
        ...
        # Apply physics
        accel_applied = t.Vadd((0, 10, 0), self.accel)
